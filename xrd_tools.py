...
def get_data(scan_num, fpath, fileformat, metadata_path, metadata_format, ai, partial_rings=False):
    filename = fpath + str(scan_num).zfill(5) + fileformat
    data = fabio.open(filename)
    
    if partial_rings:
        x, y = ai.integrate1d(data=data.data, npt=1000, polarization_factor=0.9, 
                              azimuth_range = (85, 95), radial_range=(2.5, 6.5), 
        ...
        x, y2 = ai.integrate1d(data=data.data, npt=1000, polarization_factor=0.9,
                              azimuth_range = (265, 275), radial_range=(2.5, 6.5),
                               method='bbox_cython', unit='2th_deg')
        
        y = y + y2
    
    d = 0.13865686822921588/(2*np.sin(np.deg2rad(x/2)))
    ...
    return {'2theta_deg':x, 'd_A':d, 'intensity':y, 'metadata':metadata}
...
def addLabels(ax, lax, names, hkls, positions, colours):
    ...
    yvals = 0.1 + np.arange(len(names))[::-1]*0.4
    ...
    trans2 = transforms.blended_transform_factory(lax.transAxes, lax.transAxes)
    
    for name, hkl, position, colour, yval in zip(names, hkls, positions, colours, yvals):
        
        xdata = [i.get_data()[0] for i in ax.lines]
        
        lax.text(x = 0, y=yval, s=name, fontsize=10, c=colour, fontweight='bold', ha='right', va= 'center', transform = trans2)

        for idx, angle in zip(hkl, position):
            if xmin < angle <xmax:
                ax.axvline(angle, alpha=0.2, c=colour)
                lax.axvline(angle, ymax = yval*0.75, alpha=0.2, c=colour)
